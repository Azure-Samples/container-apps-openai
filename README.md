...
The execution history for scheduled and event-based applications is limited to the most recent `100` successful and failed container app executions. To list all the executions of a container app or to get detailed output from a container app, you can query the logs provider configured for your Container Apps environment, for example [Azure Monitor Log Analytics](https://learn.microsoft.com/en-us/azure/container-apps/observability).

You can also use the Azure Portal to see the logs generated by a container app, as shown in the following screenshot. You can eventually customize the [Kusto Query Language (KQL)](https://learn.microsoft.com/en-us/azure/data-explorer/kusto/query/) query to filter, project, and retrieve only the desired data.

![Logs](images/container applogs.png)

You can use the Azure Portal, as shown in the following picture, to see the execution history for a given container app, as shown in the following picture.

![Execution History](images/executionhistory.png)

You can click the `View logs` link to see the logs for a specific container app execution, as shown in the following picture.
